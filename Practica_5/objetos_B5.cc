...
void _triangulos3D::draw(_modo modo, float r1, float g1, float b1, float r2, float g2, float b2, float grosor)
{
  switch (modo){
    ...
    case SELECT:draw_seleccion(g1); break;
	}
}
...
void _final::draw(_modo modo, float r1, float g1, float b1, float r2, float g2, float b2, float grosor){
  ...
  if(activo[0] == 1)
    tronco.draw(modo, r_s, g_s, b_s, r2, g2, b2, grosor);
  else
  ...
  glScalef(0.5, 0.5, 0.5);
  if(activo[1] == 1)
    tronco_inferior.draw(modo, r_s, g_s, b_s, r2, g2, b2, grosor);
  else
  ...
  if(activo[2] == 1)
    paisaje.draw(modo, r_s, g_s, b_s, r2, g2, b2, grosor);
  else
    paisaje.draw(modo, r1, g1, b1, r2, g2, b2, grosor);
  glPopMatrix();
}

void _final::seleccion(float c){
  // *********** Torso ***********  
  glPushMatrix();
  glTranslatef(0.0, 0.45, 0);
  glTranslatef(0.0, -0.3, 0.0);
  ...
  // glTranslatef(-0.65, -0.0, 0.0);
  glScalef(0.5, 0.5, 0.5);
  tronco_inferior.draw(SELECT, c, c, c, c, c, c, 1);
  glPopMatrix();
  ...
  glPushMatrix();
  glTranslatef(0.0, 0.5, 0);
  paisaje.draw(SELECT, c, c, c, c, c, c, 1);
  glPopMatrix();
}
